...
class SuanpanGateway extends TypertRemoteService {
  ...
  /**
   * 拉取三家归一化快照。每家独立容错：
   * 单家失败（key 失效 / API 抖动 / 超时）只在快照里标记 error，
   * 不整体抛错——UI 端单家显示 n/a，其余两家照常显示。
   */
  async snapshot() {
    const credentials = this.ctx.credentials;
    const [balance, miniMaxUsage, openCodeUsage] = await Promise.all([
      deepseek.fetchBalanceSnapshot(credentials).then(
        (value) => ({ ok: true, value }),
        (error) => ({ ok: false, error: error instanceof Error ? error.message : String(error) })
      ),
      minimax.fetchUsageSnapshot(credentials).then(
        (value) => ({ ok: true, value }),
        (error) => ({ ok: false, error: error instanceof Error ? error.message : String(error) })
      ),
      opencode.fetchUsageSnapshot(credentials).then(
        (value) => ({ ok: true, value }),
        (error) => ({ ok: false, error: error instanceof Error ? error.message : String(error) })
      )
    ]);
    return {
      deepseek: balance.ok ? balance.value : { error: balance.error },
      minimax: miniMaxUsage.ok ? miniMaxUsage.value : { error: miniMaxUsage.error },
      opencode: openCodeUsage.ok ? openCodeUsage.value : { error: openCodeUsage.error }
    };
  }
}
