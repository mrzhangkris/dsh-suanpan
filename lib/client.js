 ...
 *
 * 唯一 UI：composer 右下角读条（conversation.input.right），
 * 共用 host 端 `suanpan/snapshot` 归一化快照：
 * 按当前会话选中的 provider 自动切换显示对应通道——
 * deepseek-official → 余额+峰谷 / minimax-cn → Coding Plan 配额 /
 * opencode-go → Zen Go 配额；其他 provider 不渲染。
 *
 * 悬浮窗/用量面板由 dsh-usage 插件提供（widget 系统可融合多数据源），
 * 本插件不重复实现。
 *
 * 借鉴：dsh-usage-minimax-cn（jooey, MIT）的 composer 读条与 Typert 挂载。
